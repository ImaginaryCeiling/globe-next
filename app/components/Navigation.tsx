...
export default function Navigation({ isOpen, onToggle }: NavigationProps) {
  ...
  return (
    <>
      ...
      <div
        className={`hidden md:flex fixed left-0 top-0 h-full ${width} bg-black/90 backdrop-blur-sm border-r border-zinc-800 flex-col z-50 pointer-events-auto transition-all duration-300`}
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`flex flex-col items-center justify-center gap-1 py-2 px-4 rounded-lg min-w-[64px] ${
                  isActive
                    ? 'text-blue-400'
                    : 'text-zinc-500'
                }`}
          ...
          >
            <div className="w-6 h-6 rounded-full bg-zinc-800 flex items-center justify-center overflow-hidden border border-zinc-700">
              {user?.user_metadata?.avatar_url ? (
                ...
                <span className="text-zinc-400 font-bold text-[8px]">
                  {user?.email?.[0].toUpperCase() ?? 'U'}
                </span>
              )}
            </div>
